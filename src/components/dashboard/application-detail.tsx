...
export default function ApplicationInfo({
  ...
  const renderSubTitle = (title: string) => {
    return (
      <h2 className="select-none text-light-300 dark:text-dark-300">{title}</h2>
    );
  };

  const renderHighlightString = (content: string) => {
    return <span className="text-light-100 dark:text-dark-100">{content}</span>;
  };
  ...
  return (
    <div className="flex flex-col space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          ...
          <FavoriteToggle
            ...
            onToggle={() =>
              handleUpdateField('isFavorite', !applicationDetail.isFavorite)
            }
          />
        </div>
      </div>

      <div className="flex items-center justify-between text-light-200 dark:text-dark-200">
        <div className="flex items-center space-x-4">
          <PopoverForm
      ...
      <div className="flex flex-col space-y-2">
        ...
        <div className="ml-3 flex items-center space-x-2">
          <PopoverForm
            ...
            popoverTriggerComponent={
              <button className="rounded-sm transition-transform active:scale-95">
      ...
      <div className="flex flex-col space-y-2">
        ...
        <ul className="ml-3 flex flex-col space-y-2">
          {applicationDetail.statusTrackingLinks.map((link, index) => (
            <li key={index} className="flex items-center space-x-2">
              <PopoverForm
                ...
                onConfirm={async (newValue) => {
                  ...
                  await handleUpdateField('statusTrackingLinks', updatedLinks);
                }}
                popoverTriggerComponent={
                  <button className="rounded-sm transition-transform active:scale-95">
                    <AnchorIcon width={18} className="mx-1" />
              ...
              />
              <NextUiLink isExternal underline="hover" href={link}>
                <span className="max-w-[44rem] break-words">{link}</span>
              </NextUiLink>
            </li>
          ))}
        </ul>
