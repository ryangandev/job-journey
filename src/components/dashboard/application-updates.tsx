...
export default function ApplicationUpdates({
  ...
  return (
    <div className="flex flex-col space-y-6">
      <h2 className="select-none text-light-300 dark:text-dark-300">
        Application Updates
      </h2>
      <ul className="ml-3">
        {applicationUpdates.map((update, index) => (
          <li className="flex flex-col" key={update.id}>
            <div className="flex flex-row items-center space-x-2">
              <span className="text-lg">{updateTypeIconMap[update.type]}</span>
              <span className="space-x-2 text-sm">
                <span className="text-light-100 dark:text-dark-100">
                  {update.content}
                </span>
                <span className="select-none text-light-100 dark:text-dark-100">
                  ·
                </span>
                <span className="select-none text-sm text-light-400 dark:text-dark-400">
            ...
            {index !== applicationUpdates.length - 1 && (
              <Divider className="my-1 ml-2 h-4" orientation="vertical" />
            )}
          </li>
        ))}
      </ul>
          ...
          <Controller
            ...
            render={({ field }) => (
              <CustomDropdown
                ...
                displayMapper={(value) =>
                  jobUpdateTypeMap[value as ApplicationUpdateType]
                }
                startContent={
